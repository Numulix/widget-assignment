...
describe('useSelectionEditor', () => {
    it('seeds the draft from the initial selection', () => {
        const { result } = renderHook(() => useSelectionEditor(items, [items[0]]));
    ...
    it('toggles an item in and out of the draft', () => {
        const { result } = renderHook(() => useSelectionEditor(items, []));
    ...
    it('disabled unselected rows when full, but keeps selected ones toggleable', () => {
        const { result } = renderHook(() => useSelectionEditor(items, [items[0], items[1], items[2]]));
        expect(result.current.isFull).toBe(true);
        expect(result.current.isDisabled('item-4')).toBe(true);
        expect(result.current.isDisabled('item-1')).toBe(false);
    });

    it('removes an item from the draft', () => {
        const { result } = renderHook(() => useSelectionEditor(items, [items[0], items[1]]));
        act(() => result.current.remove('item-1'));
        expect(result.current.draft.map(i => i.id)).toEqual(['item-2']);
    });
});